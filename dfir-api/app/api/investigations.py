...
async def quarantine_instances(req: QuarantineRequest, db: AsyncSession = Depends(get_db)):
    # Accept a single instance_id for quarantine
    return await quarantine_instance([req.instance_id], req.case_number, db)


@router.post("/un-Quarantine")
async def unquarantine_instances(req: UnQuarantineRequest, db: AsyncSession = Depends(get_db)):
    return await un_quarantine_instance([req.instance_id], req.case_number, db)
...
async def create_case(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    new_case = Case(
        ...
        assigned_to=body.get("accountName"),  # Use accountName for assigned_to
    ...
    # Set accountName after creation, not as a keyword argument
    if hasattr(new_case, "accountName"):
        setattr(new_case, "accountName", body.get("accountName"))
    db.add(new_case)
    ...
    # Return all fields of the new_case as a dict, plus a message
    response = {"message": "Case created successfully"}
    response.update({
        k: getattr(new_case, k)
        for k in [
            "case_number", "title", "description", "priority", "status", "assigned_to", "created_at", "updated_at", "due_date", "instance_id", "accountId", "resource_type", "cloud"
        ]
        if hasattr(new_case, k)
    })
    return response
...
async def get_open_cases(db: AsyncSession = Depends(get_db)):
    ...
    result = await db.execute(
        select(Case).where(Case.status.notin_(["Failed", "Error"]))
    )
...
async def get_nkase_action(request: Request, session: AsyncSession = Depends(get_db)):
    try:
        ...
        process_task = CaseTask(
            ...
            due_date=None,  # Set if you have a due date
            notes=f"Malware analysis process started for instance {case.instance_id} (Account: {case.accountId}). Instance ID: {case.instance_id}, Account ID: {case.accountId}",
            completed_at=None,
            completed_by=None,
        ...
        try:
            ...
            await set_status(
                ...
                stage="Forensic Malware analysis sarted",
                status="Forensic Progress",
                ...
                errors=None,
                details={
                    "note": f"Mitigation started for instance id: {instanceid} with job id: {job_id}"
                }
            )

            await redis.enqueue_job("mitigation_worker", instanceid, accountId, case_number, job_id)

            # Call set_status to log the start of mitigation
...
@router.websocket("/ws/investigations/nkase/logs")
async def websocket_malware_logs(websocket: WebSocket):
    await websocket.accept()
    from app.db.database import async_session_maker
    session = None
    try:
        # Get case_number from query params
        ...
        while True:
            ...
            if new_logs:
                for log in new_logs:
                    ...
                    await websocket.send_text(json.dumps(log_dict))
                last_id = new_logs[-1].id
            await asyncio.sleep(2)
